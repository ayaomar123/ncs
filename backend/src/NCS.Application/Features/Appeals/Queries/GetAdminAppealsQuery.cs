...
public sealed record GetAdminAppealsQuery(
    int PageNumber = 1,
    int PageSize = 10,
    bool? IsUrgent = null,
    string? CountryTag = null) : IRequest<PagedResult<AppealDto>>;

public sealed class GetAdminAppealsQueryHandler(IAppealRepository repository) : IRequestHandler<GetAdminAppealsQuery, PagedResult<AppealDto>>
{
    public async Task<PagedResult<AppealDto>> Handle(GetAdminAppealsQuery request, CancellationToken cancellationToken)
    {
